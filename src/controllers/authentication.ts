...
class Authentication {
  ...
  validateLoginAttempt = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { email, password } = req.body;

      // Check if email and password are provided
      if (!email || !password) {
        return next(new AppError("Missing information: you must provide both email and password.", 400));
      }

      // Retrieve the user by email, ensure the user is active, and select the required fields
      const user = await User.findOne({ email }).select(["+password", "+loginAttempts", "+loginExpires", "+lastLoginAttempt", "+isVerified"]);
      ...
      if (!user.isVerified) {
        return next(new AppError("You account is not verified yet, please check your email for verification link.", 401));
      }

      // Check login attempt limits
      if (!user.checkLogin()) {
        await user.save(); // Save the state in case login attempt count or lockout has been updated
        const minutesRemaining = (((user.loginExpires as unknown as number) - Date.now()) / 1000 / 60).toFixed();

        return next(new AppError(`You have reached the maximum login attempts, please try again in ${minutesRemaining} Minutes.`, 401));
      }
  ...
  generateToken(data: any) {
    ...
    const expiresInValue = isNaN(expiresIn as unknown as number) ? expiresIn : parseInt(expiresIn, 10);
  ...
  protect = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user.isVerified) {
        return next(new AppError("You account is not verified, please check your email for verification link. ", 401));
      }

      // check if the user changed the password and the token is issued after user changing it
      if (user.checkChangedPassword(iat)) {
        return next(new AppError("User changed the password, login again to get a new token.", 401));
      }
  ...
  updatePassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // check for passwords
      if (!password || !newPassword) return next(new AppError("Missing information you have to provide password and newPassword", 400));
